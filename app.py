...
with st.container():
   ...
   # --- Contenedor Abajo: Cálculo de Kilómetros Recorridos ---
st.markdown("---")
with st.container():
    with st.expander("4. Calculadora de Kilómetros Recorridos", expanded=True):
        st.subheader("Kilómetros totales recorridos por cada técnico (por red vial)")
        ...
        # URL del servicio de enrutamiento OSRM
        OSRM_URL = "http://router.project-osrm.org/route/v1/driving/"

        # 1. Función para calcular la distancia por la red vial usando la API de OSRM
        @st.cache_data
        def get_driving_distance(origin, destination):
            """
            Calcula la distancia de conducción entre dos puntos usando la API de OSRM.
            """
            try:
                # OSRM espera las coordenadas en formato 'longitud,latitud'
                origin_str = f"{origin[1]},{origin[0]}"
                destination_str = f"{destination[1]},{destination[0]}"
                query = f"{OSRM_URL}{origin_str};{destination_str}"
                ...
                if data['code'] == 'Ok' and data['routes']:
                    # La distancia se devuelve en metros, la convertimos a kilómetros
                    return data['routes'][0]['distance'] / 1000
                else:
                    st.warning(f"No se pudo encontrar una ruta para la ubicación {destination}.")
                    return 0
            except requests.exceptions.RequestException as e:
                st.error(f"Error al conectar con la API de OSRM: {e}")
                return 0
            except Exception as e:
                st.error(f"Error inesperado en el cálculo de distancia: {e}")
                return 0

        # 2. Aplicar la función a cada fila del DataFrame
        def calculate_distance(row):
            if pd.notna(row['Coord Y']) and pd.notna(row['Coord X']):
                destination_coords = (row['Coord Y'], row['Coord X'])
                return get_driving_distance(base_coords, destination_coords)
            return 0

        # Verificar si las columnas existen antes de calcular
        if 'Coord Y' in filtered_df.columns and 'Coord X' in filtered_df.columns:
            # st.info("Calculando distancias por red vial, esto puede tardar un momento...")
            filtered_df['Distancia_km'] = filtered_df.apply(calculate_distance, axis=1)

            # 3. Sumar el total de kilómetros por técnico
            if selected_tecnico_filter == 'TODOS':
                total_km_por_tecnico = filtered_df.groupby('Tecnico')['Distancia_km'].sum().reset_index()
                ...
                total_general_km = total_km_por_tecnico['Total Kilómetros Recorridos'].sum()
                st.markdown(f"### **Total General de Kilómetros:**")
                st.success(f"**{total_general_km:,.2f} km**")
            else:
                total_km = filtered_df['Distancia_km'].sum()
                st.markdown(f"### **Kilómetros Totales para {selected_tecnico_filter}:**")
                st.success(f"**{total_km:,.2f} km**")
        else:
            st.warning("No se encontraron las columnas 'Coord X' y/o 'Coord Y' en los datos filtrados.")